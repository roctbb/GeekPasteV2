...
def get_payload(task_text, solution_text, max_points, lang=None, check_ai=False, solution_kind='code'):
    ...
    if solution_kind == 'image':
        ...
        prompt = (
            ...
            "ВАЖНО: число строк, наличие README, requirements.txt, отдельных модулей и качество "
            "игры НЕ являются условиями QUEST_SUBMISSION: YES. Даже игра в одном файле на 10 строк "
            "с сюжетом, input и выбором между двумя исходами — YES. Отсутствие README и модулей "
            "оценивается только в соответствующих остальных критериях. Правило менее 100 строк "
            "означает YES и 0 остальных баллов (итог системы 5), а не NO. "
            ...
            "Для критерия разбиения на функции достаточно не менее 5 функций, определённых "
            "в коде проекта суммарно по всем файлам: это могут быть функции локаций, действий "
            "или вспомогательные функции. При наличии 5 и более функций ставь 5 баллов, иначе 0. "
            "Отдельная функция для каждой локации не требуется; обработка локаций общей логикой "
            "в цикле или конечном автомате не снижает балл по этому критерию. "
            "Для критерия обратных переходов косвенная рекурсия и конечный автомат "
            "равноценны: достаточно работающей возможности вернуться в ранее посещённую локацию. "
            "Конечный автомат может быть реализован циклом с текущей локацией или координатами "
            "и условиями либо функциями переходов; рекурсивные вызовы не обязательны. "
            "Сам по себе цикл меню без обратных переходов между локациями этот критерий не выполняет. "
